...
namespace Language_Understanding
{
    class Program
    {
        ...
        static async Task Main(string[] args)
        {
            try
            {
                string predictionKey = "61530daa35ce4649ada47a3e9544f284";
                string predictionEndpoint = "https://aiconversationallanguag.cognitiveservices.azure.com/";

                // Create a client to access the Language Understanding model
                Uri endpoint = new Uri(predictionEndpoint);
                AzureKeyCredential credential = new AzureKeyCredential(predictionKey);

                ConversationAnalysisClient client = new ConversationAnalysisClient(endpoint, credential);

                // Prompt user for input (using "quit" to end)
                string userText = "";
                while (userText.ToLower() != "quit")
                {
                    ...
                    if (userText.ToLower() != "quit")
                    {
                        ...
                        var projectName = "AIProject";
                        var deploymentName = "SecondDeployment";
                        var data = new
                        {
                            analysisInput = new
                            {
                            ...
                            parameters = new
                            {
                                projectName,
                                deploymentName,
                                // Using Utf16CodeUnit for strings in .NET.
